...
def handle_events():
    ...
    for event in events:
        ...
        elif event.type == SDL_MOUSEBUTTONDOWN and event.button == SDL_BUTTON_LEFT: # 마우스 클릭이 있으
            points.append((event.x, TUK_HEIGHT - 1 - event.y)) # 클릭된 위치 새로운 점으로 추가
...
def set_new_target_arrow():
    ...
    if points:
        ...
        action = 3 if action == 1 else 2 # 이전에 소년이 우측으로 이동중이었으면 소년이 아이들 동작시 우측을 바라보도록
        frame = 0
        target_exists = False
